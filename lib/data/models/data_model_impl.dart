...
class DataModelImpl extends DataModel {
  ...
  Future<void> addNewMoment(
      String description, File? file, bool isVideo) {
    if (file != null) {
      print("======> with image");
      return mDataAgent
  ...
  Future<void> editMoment(MomentVO? newsFeed, File? image) {
    if(image != null){
      print("=====> with image");
      return mDataAgent.uploadFileToFirebase(image).then((downloadUrl) => craftMomentVOForEdit(newsFeed, downloadUrl)).then((value) => mDataAgent.addNewMoment(value));
    } else {
      print("=====> without image");
